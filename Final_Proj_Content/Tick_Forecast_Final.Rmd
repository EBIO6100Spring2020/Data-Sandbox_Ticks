...
summedCorrect %>%
  ...
  ggplot() + geom_line(aes(x=dayOfYear_cutoff, y=1-propCorrect, col=Predictions_for))+
  scale_color_manual(values=c("blue","black"))+
  scale_y_continuous(sec.axis = sec_axis(trans=~(.-0)*3, name = "Borrelia presence/absence"), limits=c(0,0.35)) +
  geom_point(data=tck_borrelia_test, aes(x=as.Date(dayOfYear, origin = "2017-01-01"), y=as.numeric(borrPresent)/3+0), col="red") +
  geom_smooth(data=tck_borrelia_test, aes(x=as.Date(dayOfYear, origin = "2017-01-01"), y=borrPresent/3+0), col="red") +
    theme(axis.title.y.right = element_text(colour = "red")) +ylab("Proportion predicted incorrect") 
...
summedCorrect %>%
  mutate(dayOfYear_cutoff = as.Date(doy_cutoff, origin = "2017-01-01")
         , month = month(dayOfYear_cutoff)
         , Predictions_for = ifelse(pred_type=="test_pred","Test set","Train set")) %>%
  ggplot() + geom_line(aes(x=dayOfYear_cutoff, y=propFPositive, col=Predictions_for))+
  scale_color_manual(values=c("blue","black"))+
  scale_y_continuous(sec.axis = sec_axis(trans=~.*5, name = "Borrelia presence/absence"), limits=c(0,0.2)) +
...
summedCorrect %>%
  mutate(dayOfYear_cutoff = as.Date(doy_cutoff, origin = "2017-01-01")
         , month = month(dayOfYear_cutoff)
         , Predictions_for = ifelse(pred_type=="test_pred","Test set","Train set")) %>%
  ggplot() + geom_line(aes(x=dayOfYear_cutoff, y=propFNegative, col=Predictions_for))+
  scale_color_manual(values=c("blue","black"))+
  scale_y_continuous(sec.axis = sec_axis(trans=~.*3, name = "Borrelia presence/absence"), limits=c(0,0.5)) +
  geom_point(data=tck_borrelia_test, aes(x=as.Date(dayOfYear, origin = "2017-01-01"), y=as.numeric(borrPresent)/3), col="red") +
  geom_smooth(data=tck_borrelia_test, aes(x=as.Date(dayOfYear, origin = "2017-01-01"), y=borrPresent/3), col="red") +
